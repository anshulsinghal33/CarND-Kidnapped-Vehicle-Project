...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  num_particles = 25;//2;//4;//10;//25;//100;  // TODO: Set the number of particles
  
  // normal distributions for sensor noise
  normal_distribution<double> normal_dist_x_init(0, std[0]);
  normal_distribution<double> normal_dist_y_init(0, std[1]);
  normal_distribution<double> normal_dist_theta_init(0, std[2]);
  
  weights.resize(num_particles);
  
  //init particles
  for (signed int i = 0; i < num_particles; ++i) {
    Particle p; // creating particle
    ...
    // adding noise
    p.x += normal_dist_x_init(gen);
    p.y += normal_dist_y_init(gen);
    p.theta += normal_dist_theta_init(gen);
...
void ParticleFilter::prediction(double delta_t, double std_pos[], 
  ...
  normal_distribution<double> normal_dist_theta(0, std_pos[2]);
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for (int i = 0; i < num_particles; ++i) {
    ...
    for (unsigned int j = 0; j < observations.size(); ++j) {
      ...
      // Transform observations from vehicle's co-ordinate system to map co-ordinate sytem
      ...
      for (unsigned int k = 0; k < landmarks.size(); ++k) {
        ...
        // Keep only those landmarks which are in sensor range
        ...
        if (landmark_part_dist <= sensor_range) {
          ...
          // Need to fill those outside proximity with a very large number, or they'll be a zero and will be incorrectly considered as closest.
          landmark_obs_dist[k] = std::numeric_limits<double>::max();  
        }    
      }
      ...
      double b = ((x_diff * x_diff) / x_den) + ((y_diff * y_diff) / y_den);
      mvGd *= a * exp(-b);
    }    
    // Update particle weights with combined mvGd
